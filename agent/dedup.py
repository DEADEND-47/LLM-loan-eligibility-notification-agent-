...
class DedupManager:
    ...
    def _ensure_table(self) -> None:
        ...
        try:
            ...
            if "loan_amount" not in columns:
                self.logger.info("Migrating database: adding 'loan_amount' column to notifications table")
                cursor.execute("ALTER TABLE notifications ADD COLUMN loan_amount REAL")
    ...
    def record_notification(
        ...
        Args:
            ...
            loan_amount (float, optional): Recommended loan size amount.
        """
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
            ...
            cursor.execute(
                """
                INSERT OR REPLACE INTO notifications (prospect_id, notification_id, tier, sent_at, language, status, channel, processing_status, processing_remark, loan_amount)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (prospect_id, notification_id, tier, sent_at_iso, language, status, channel, processing_status, processing_remark, loan_amount)
            )
            conn.commit()
