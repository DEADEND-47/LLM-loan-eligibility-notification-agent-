...
from agent.generator import MessageGenerator, format_compact_inr
            ...
            try:
                ...
                if pd.isna(contact_no) or str(contact_no).strip() == "":
                    self.dedup.record_notification(
                        ...
                        loan_amount=float(row.get("Recommended_Loan_Amount", 0.0))
                    )
                ...
                if eligibility_dict is None:
                    # Declined prospect (P4)
                    tier = str(row.get("Approved_Flag", "P4"))
                    self.dedup.record_notification(
                        prospect_id=prospect_id,
                        notification_id="",
                        tier=tier,
                        ...
                        channel="N/A",
                        processing_status=f"Skipped - Declined ({tier})",
                        processing_remark=f"Skipped because customer risk tier is {tier} (declined).",
                        loan_amount=float(row.get("Recommended_Loan_Amount", 0.0))
                    )
                    self.logger.warning(f"PROSPECTID {prospect_id}: Skipped - Declined ({tier}). Reason: Risk tier is {tier}.")
                    skipped += 1
                    max_processed_id = max(max_processed_id, prospect_id)
                    # Update watermark immediately (Requirement 6: crash resilience)
                    self.loader.update_watermark(prospect_id)
                ...
                is_valid, rejection_reason = validate_loan_offer(eligibility_dict)
                if not is_valid:
                    if rejection_reason == "Loan amount below minimum threshold":
                        status = "skipped_zero_loan"
                        processing_status = "Skipped - Zero Loan Amount"
                    elif rejection_reason == "EMI calculation failed or income too low":
                        status = "skipped_zero_emi"
                        processing_status = "Skipped - Zero EMI"
                    elif rejection_reason == "EMI exceeds FOIR limit":
                        status = "skipped_foir_exceeded"
                        processing_status = "Skipped - FOIR Exceeded"
                    elif rejection_reason == "Income below minimum threshold":
                        status = "skipped_income_below_floor"
                        processing_status = "Skipped - Income Below Floor"
                    else:
                        status = "skipped_declined"
                        processing_status = "Skipped - Declined"
                    ...
                    self.dedup.record_notification(
                        ...
                        processing_remark=rejection_reason,
                        loan_amount=eligibility_dict["loan_amount"]
                    )
                    self.logger.warning(
                        f"PROSPECTID {prospect_id}: Skipped. Reason: {rejection_reason} ({processing_status})."
                    )
                    skipped += 1
                    max_processed_id = max(max_processed_id, prospect_id)
                    # Update watermark immediately (Requirement 6: crash resilience)
                ...
                try:
                    ...
                    self.logger.error(f"Twilio error {e.code}: {e.msg} → recipient: {contact_str}")
                    self.dedup.record_notification(
                        prospect_id=prospect_id,
                        notification_id=notification_id,
                        tier=eligibility_dict["tier"],
                        ...
                        channel="WhatsApp",
                        processing_status="Failed - Twilio Error",
                        processing_remark=f"Twilio error {e.code}: {e.msg}",
                        loan_amount=eligibility_dict["loan_amount"]
                    )
                    errors += 1
                    max_processed_id = max(max_processed_id, prospect_id)
